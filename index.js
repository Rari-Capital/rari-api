    ...
    for (const event of events) {
        ...
        if (!exchangeRate) {
            try {
                ...
                else var fundBalanceBN = Web3.utils.toBN(await contractsWithArchive[poolKey].RariFundManager.methods.getFundBalance().call(event.blockNumber));
            } catch (error) {
...
async function startCheckingForBlocks(startBlockNumber) {
    ...
    for (var i = startBlockNumber; i <= endBlockNumber; i++) {
        ...
        for (const poolKey of ["yield", "ethereum"]) {
            ...
            try {
                var fundBalance = await contractsWithArchive[poolKey].RariFundManager.methods.getFundBalance().call(i);
            } catch (error) {
                console.error("Failed to get fund balance at block #" + i + ":", error);
                return setTimeout(resetCheckingForBlocks, 15 * 60000);
            }
